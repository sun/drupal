...
class FieldValidationTest extends FieldUnitTestBase {
  ...
  function testCardinalityConstraint() {
    $cardinality = $this->fields->field_storage->cardinality;
    $entity = $this->entity;

    for ($delta = 0; $delta < $cardinality + 1; $delta++) {
      $entity->{$this->fields->field_name}->get($delta)->set('value', 1);
    }

    // Validate the field.
    $violations = $entity->{$this->fields->field_name}->validate();

    // Check that the expected constraint violations are reported.
    $this->assertEqual(count($violations), 1);
    $this->assertEqual($violations[0]->getPropertyPath(), '');
    $this->assertEqual($violations[0]->getMessage(), t('%name: this field cannot hold more than @count values.', array('%name' => $this->fields->instance->getLabel(), '@count' => $cardinality)));
  }
  ...
  function testFieldConstraints() {
    ...
    for ($delta = 0; $delta < $cardinality; $delta++) {
      ...
      if ($delta == 1) {
        ...
        $value = -1;
        $expected_violations[$delta . '.value'][] = t('%name does not accept the value -1.', array('%name' => $this->fields->instance->getLabel()));
      }
      $entity->{$this->fields->field_name}->get($delta)->set('value', $value);
    }
