...
class FieldAttachOtherTest extends FieldUnitTestBase {
  ...
  function testEntityDisplayBuild() {
    ...
    $entity_init->{$this->fields->field_name}->setValue($values);
    $values_2 = $this->_generateTestFieldValues($this->fields->field_storage_2->getCardinality());
    $entity_init->{$this->fields->field_name_2}->setValue($values_2);
    ...
    $display->setComponent($this->fields->field_name, $display_options);
    ...
    $display->setComponent($this->fields->field_name_2, $display_options_2);

    // View all fields.
    $content = $display->build($entity);
    $this->render($content);
    $this->assertRaw($this->fields->instance->getLabel(), "First field's label is displayed.");
    foreach ($values as $delta => $value) {
      $this->assertRaw("$formatter_setting|{$value['value']}", "Value $delta is displayed, formatter settings are applied.");
    }
    $this->assertRaw($this->fields->instance_2->getLabel(), "Second field's label is displayed.");
    ...
    $content = $display->build($entity);
    $this->render($content);
    $this->assertNoRaw($this->fields->instance->getLabel(), "Hidden label: label is not displayed.");

    // Field hidden.
    $entity = clone($entity_init);
    $display->removeComponent($this->fields->field_name);
    $content = $display->build($entity);
    $this->render($content);
    $this->assertNoRaw($this->fields->instance->getLabel(), "Hidden field: label is not displayed.");
    foreach ($values as $delta => $value) {
    ...
    $entity = clone($entity_init);
    $formatter_setting = $this->randomName();
    $display->setComponent($this->fields->field_name, array(
      'label' => 'above',
  ...
  function testEntityCache() {
    ...
    $entity_init = entity_create('entity_test', array('type' => $this->fields->instance->bundle));
    $values = $this->_generateTestFieldValues($this->fields->field_storage->getCardinality());
    ...
    $entity = clone($entity_init);
    $entity->{$this->fields->field_name}->setValue($values);
    ...
    // Save, and check that no cache entry is present.
    $entity = clone($entity_init);
    $entity->{$this->fields->field_name_2} = $values;
    $entity->save();
    $cid = "values:$entity_type:" . $entity->id();
    ...
    $values = $this->_generateTestFieldValues($this->fields->field_storage_2->getCardinality());
    $entity->{$this->fields->field_name_2} = $values;
    $entity->save();
  ...
  function testEntityFormDisplayBuildForm() {
    ...
    $entity_type = 'entity_test';
    $entity = entity_create($entity_type, array('id' => 1, 'revision_id' => 1, 'type' => $this->fields->instance->bundle));

    // Test generating widgets for all fields.
    $display = entity_get_form_display($entity_type, $this->fields->instance->bundle, 'default');
    $form = array();
    $form_state = \Drupal::formBuilder()->getFormStateDefaults();
    $display->buildForm($entity, $form, $form_state);

    $this->assertEqual($form[$this->fields->field_name]['widget']['#title'], $this->fields->instance->getLabel(), "First field's form title is {$this->fields->instance->getLabel()}");
    $this->assertEqual($form[$this->fields->field_name_2]['widget']['#title'], $this->fields->instance_2->getLabel(), "Second field's form title is {$this->fields->instance_2->getLabel()}");
    for ($delta = 0; $delta < $this->fields->field_storage->getCardinality(); $delta++) {
      // field_test_widget uses 'textfield'
      $this->assertEqual($form[$this->fields->field_name]['widget'][$delta]['value']['#type'], 'textfield', "First field's form delta $delta widget is textfield");
    }
    for ($delta = 0; $delta < $this->fields->field_storage_2->getCardinality(); $delta++) {
      // field_test_widget uses 'textfield'
      $this->assertEqual($form[$this->fields->field_name_2]['widget'][$delta]['value']['#type'], 'textfield', "Second field's form delta $delta widget is textfield");
    }

    // Test generating widgets for all fields.
    $display = entity_get_form_display($entity_type, $this->fields->instance->bundle, 'default');
    foreach ($display->getComponents() as $name => $options) {
      if ($name != $this->fields->field_name_2) {
    ...
    $this->assertFalse(isset($form[$this->fields->field_name]), 'The first field does not exist in the form');
    $this->assertEqual($form[$this->fields->field_name_2]['widget']['#title'], $this->fields->instance_2->getLabel(), "Second field's form title is {$this->fields->instance_2->getLabel()}");
    for ($delta = 0; $delta < $this->fields->field_storage_2->getCardinality(); $delta++) {
      // field_test_widget uses 'textfield'
      $this->assertEqual($form[$this->fields->field_name_2]['widget'][$delta]['value']['#type'], 'textfield', "Second field's form delta $delta widget is textfield");
  ...
  function testEntityFormDisplayExtractFormValues() {
    ...
    $entity_type = 'entity_test';
    $entity_init = entity_create($entity_type, array('id' => 1, 'revision_id' => 1, 'type' => $this->fields->instance->bundle));
    ...
    for ($delta = 0; $delta < $this->fields->field_storage->getCardinality(); $delta++) {
      ...
      do {
        $weight = mt_rand(0, $this->fields->field_storage->getCardinality());
    ...
    for ($delta = 0; $delta < $this->fields->field_storage_2->getCardinality(); $delta++) {
      ...
      // Assign random weight.
      do {
        $weight = mt_rand(0, $this->fields->field_storage_2->getCardinality());
    ...
    $form_state['values'][$this->fields->field_name] = $values;
    $form_state['values'][$this->fields->field_name_2] = $values_2;
    ...
    $this->assertIdentical($entity->{$this->fields->field_name}->getValue(), $expected_values, 'Submit filters empty values');
    foreach ($weights_2 as $key => $value) {
      if ($key != 1) {
        $expected_values_2[] = array('value' => $values_2[$key]['value']);
      }
    }
    $this->assertIdentical($entity->{$this->fields->field_name_2}->getValue(), $expected_values_2, 'Submit filters empty values');
    ...
    $this->assertTrue($entity->{$this->fields->field_name}->isEmpty(), 'The first field is empty in the entity object');
    $this->assertIdentical($entity->{$this->fields->field_name_2}->getValue(), $expected_values_2, 'Submit filters empty values');
  }
